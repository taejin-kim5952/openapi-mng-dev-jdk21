...
const fn_api_paramtest_object = {
  ...
  fn_get_udf_param_object: (function(rs_param) {
    return {
      ...
      't' : KsmUtil.$sf_obj_val(rs_param, 'datatype_name', ''),
  ...
  fn_get_rs_param_object: (function(udf_param) {
    return {
      ...
      'datatype_name': KsmUtil.$sf_obj_val(udf_param, 't', '').toLowerCase(),
  ...
  fn_get_param_node: (function(param_obj, paramnmfull, datatypecdnm) {
    let a_paramnmfull = paramnmfull.split('.');
    
    let obj = param_obj;
    for (let n_ii = 0; n_ii < a_paramnmfull.length; n_ii++) {
      let paramnm = a_paramnmfull[n_ii];
      if (KsmUtil.$has_own(obj, paramnm) == false) {
        //--[chk][break;가 아닌이유]
        continue;
      }
      let obj_child = obj[paramnm];
      if ((obj_child != null) && ('object' == typeof(obj_child))) { //-- [i]assert(typeof(null) == 'object');
        obj = obj_child;
        if (Array.isArray(obj_child) == true) {
          if (obj_child.length > 0) { 
            ...
            let param_def_val = this.fn_get_param_def_val(datatypecdnm, null);
            if (param_def_val != null) {
              obj_child.push(param_def_val);  //-- 기본값이있는경우만 추가
            }
          }
        }
      }
  ...
  fn_get_api_param_object: (function(a_udf_param, is_set_testdata) {
    ...
    for (let n_ii = 0; n_ii < a_udf_param.length; n_ii++) {
      ...
      let datatype_name = udf_param['t'];
      //-- [tag:object_in_array]
      if ('Object in Array' == datatype_name) { 
        //--[i][implicit] param_def_val = this.fn_get_param_def_val('object');
        datatypecdnm = null; //-- [i]for no default param setting in fn_get_param_node()
      }
      ...
      if (paramnmfull == paramnm) { //-- root node
        ...
        let param_node = this.fn_get_param_node(api_param_obj, paramnmfull, datatypecdnm);
        if ((param_node != null) && ('object' == typeof(param_node))) { //-- [i]assert(typeof(null) == 'object');
  ...
  fn_assign_testcase_value: (function(cpapireq_param, a_param_testcase, a_udf_param) {
    ...
    for (let n_ii = 0; n_ii < a_param_testcase.length; n_ii++) {
      ...
      if (paramnmfull.length > 0) {
        ...
        a_paramnmfull.splice(-1, 1);  //-- 맨뒤삭제
        let parent_paramnmfull = a_paramnmfull.join('.'); //-- parent paramnmfull
        if (parent_paramnmfull.length == 0) {
          ...
          let parent_param_node = this.fn_get_param_node(cpapireq_param, parent_paramnmfull);
          if ((parent_param_node != null) && ('object' == typeof(parent_param_node))) { //-- [i]assert(typeof(null) == 'object');
            parent_param_node[paramnm] = value;
          }
        }
      }
    }
